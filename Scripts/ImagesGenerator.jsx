  ...
  ImagesGenerator.prototype = {
    ...
    UIcreateWindow: function() {
      ...
      panelTemplateBtn.addEventListener('click', function() {
        ...
        self.saveFolderName = self.inputFile.path.replace(/_templates/, 'Exports/Source_files/')
      ...
      if (folderMain.exists) {
        ...
        for (var i = 0; i < subFoldersList.length; ++i) {
          ...
          if (subFoldersList[i] instanceof Folder) {
            jsonData = this.readJSON(this.languageFolderName + languageName + '/images.json')
            if (jsonData) {
    ...
    readJSON: function(jsonFilePath) {
      ...
      if (json.exists) {
        ...
        alert('Error parsing JSON for: ' + jsonFilePath + '\nPlease check that all language folders are lowercase and 2 characters long (e.g: "en/")')
      }
    },
